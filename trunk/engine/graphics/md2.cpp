...
bool CMD2Model::LoadModel( const char *filename )
{
	...
	{
		m_bboxes[j][0] = 9999.9f;
		m_bboxes[j][1] = 9999.9f;
		m_bboxes[j][2] = 9999.9f;
		m_bboxes[j][3] = -9999.9f;
		m_bboxes[j][4] = -9999.9f;
		m_bboxes[j][5] = -9999.9f;
	}
	...
	for( int j = 0; j < num_frames; j++ )
	{
		...
		for( int i = 0; i < num_xyz; i++ )
		{
			...
			if( ptrverts[i][0] > m_bboxes[j][3] )
				m_bboxes[j][3] = ptrverts[i][0];

			if( ptrverts[i][1] < m_bboxes[j][1] )
				m_bboxes[j][1] = ptrverts[i][1];

			if( ptrverts[i][1] > m_bboxes[j][4] )
				m_bboxes[j][4] = ptrverts[i][1];

			if( ptrverts[i][2] < m_bboxes[j][2] )
				m_bboxes[j][2] = ptrverts[i][2];
...
// ----------------------------------------------
// InterpolateBB() - interpolate BoundingBoxes
// ----------------------------------------------
void CMD2Model::GetInterpolatedBoundingBox( box_t bbox )
{
	box_t	*curr_b;	// pointeur to current bbox
	box_t	*next_b;	// pointeur to next bbox

	// Interpolate bounding boxes
	curr_b = &m_bboxes[ m_anim.curr_frame ];
	next_b = &m_bboxes[ m_anim.next_frame ];

	// NOTA: al estar el modelo rotado (ver los glRotate() en DrawFrame()) tenemos
	// que transponer la coordenadas:
	//
	// MD2.X = SMall.Z,   MD2.Y = SMall.X,   MD2.Z = SMall.Y 
	bbox[2] = (curr_b[0][0] + m_anim.interpol * (next_b[0][0] - curr_b[0][0])) * m_scale;
	bbox[0] = (curr_b[0][1] + m_anim.interpol * (next_b[0][1] - curr_b[0][1])) * m_scale;
	bbox[1] = (curr_b[0][2] + m_anim.interpol * (next_b[0][2] - curr_b[0][2])) * m_scale;

	bbox[5] = (curr_b[0][3] + m_anim.interpol * (next_b[0][3] - curr_b[0][3])) * m_scale;
	bbox[3] = (curr_b[0][4] + m_anim.interpol * (next_b[0][4] - curr_b[0][4])) * m_scale;
	bbox[4] = (curr_b[0][5] + m_anim.interpol * (next_b[0][5] - curr_b[0][5])) * m_scale;
}
...
void CMD2Model::RenderFrame( void )
{
	...
	glPushAttrib( GL_POLYGON_BIT );
	{
		...
		// PENDIENTE: esto se har�de forma global una vez por frame  no ?
		...
		// HACK: deshabilitamos la textura
